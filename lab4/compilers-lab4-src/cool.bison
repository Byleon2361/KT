...
| CLASS TYPEID INHERITS TYPEID '{' feature_list '}' ';'  /* Добавлено правило для наследования */
...
| WHILE expr LOOP expr POOL  /* Добавлено правило для цикла while */
  { $$ = loop($2, $4); }
...
| expr '*' expr
  { $$ = mul($1, $3); }
| expr '/' expr  /* Добавлено правило для деления */
  { $$ = divide($1, $3); }
| '~' expr
  { $$ = neg($2); }
| expr '<' expr
