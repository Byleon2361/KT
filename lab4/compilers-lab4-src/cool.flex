...
#define yylval cool_yylval //  Если токен имеет связанное с ним значение , yylex() сохраняет это значение в глобальной переменной yylval.
#define yylex  cool_yylex //  возвращает целочисленный код, представляющий тип распознанного токена.
...
<STRING>\" {
    ...
    cool_yylval.symbol = stringtable.add_string((char*)output.c_str()); // Раньше тут был TODO
...
[ \f\r\t\v]+ { }
 ...
 /* TYPEID */
[A-Z][A-Za-z0-9_]* {
    cool_yylval.symbol = idtable.add_string(yytext); //Раньше тут был TODO
    return TYPEID;
}
